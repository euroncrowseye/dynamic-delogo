...
def draw_blur(frame, x, y):
  padded_frame = np.zeros(shape=(clip.h+pad,clip.w+pad,3),dtype=uint8)
...
def process_frames(get_frame,t):
  ...
  if t*fps/everyf - np.fix(t*fps/everyf) < 1 / everyf:
    ...
    for x in range(tsteps-1):
      tmax[x],box[x] = match(favg[x].astype(np.uint8),logo)
    for x in range(maxtsteps):
      tmax[tsteps-1],box[tsteps-1] = match(favg[tsteps-1].astype(np.uint8),logo)
